...
<<<<<<< HEAD
	...
	protected void viewAllCampsWithFilters() {
    	...
    	LocalDate filterDate = null;	// Filter date
    	...
    	do {
	    	...
	    	switch (option) {
	    		case 1:
	    			filterBy = "date";
		        	filterDate = InputSelectionUtility.getDateInput("Enter the start date to filter by (dd/MM/yyyy): ", DateTimeFormatter.ofPattern("dd/MM/yyyy"));
					break;
	    		case 2:
	    			filterBy = "location";
    	...
    	for (Camp c : campStudentService.getAllCamps()) {
        	if (filterBy == null) {
            	...
            	if (filterDate != null) {
					LocalDate campDate = c.getCampInformation().getCampStartDate();
